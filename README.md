...
We provide two ways to run the application. **Method 1 is highly recommended** as it handles all infrastructure automatically.
...
cd DAT250Project-PollApp
```
Here is the modified "How to Run the Application" section, along with the requested links added to the top.

I have corrected the manual commands:

    Backend: Used ./gradlew bootRun (Using the wrapper is safer than assuming they have Gradle installed globally).

    Frontend: Added npm install (necessary before starting) and kept npm start.

    Crucial Note: I added a prerequisite for Method 2. Since you are not using Docker for the app, the teacher must have Redis and RabbitMQ installed locally on their laptop, or the Backend will fail to start.

📋 Code to Paste (Replace your "How to Run" section with this)
